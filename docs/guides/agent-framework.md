...
On start, the adapter sends a subscribe-only frame (`sendSubscribe(sessionId, this.lastSequence)`) so the runtime replays accepted envelopes for the session from the adapter's own cursor before switching to live broadcast ([RFC-MACP-0006 (Transport Bindings)](https://github.com/multiagentcoordinationprotocol/multiagentcoordinationprotocol/blob/main/rfcs/RFC-MACP-0006-transport-bindings.md) §3.2). `lastSequence` is `0` before anything is delivered, so a fresh adapter's first `start()` still replays the whole session — that is what lets a non-initiator `Participant` attach to a session at any point and still see the `SessionStart` + prior `Proposal` / `Vote` / … envelopes, regardless of spawn order or connection timing. A second `start()` on the same adapter (after `stop()`) resumes from where the first pass left off instead of replaying from the beginning; the adapter tracks that cursor by counting distinct accepted envelopes (keyed on `message_id`), not raw deliveries, so a redelivered envelope does not advance it. That protection is scoped to the cursor only, not to your code: dispatch to user-registered handlers (`Participant.on`, `src/agent/participant.ts:345`) has no `message_id` filter of its own, so a redelivered envelope still reaches your handlers a second time and handler logic needs to be idempotent on its own terms.

Note that this adapter does not reconnect on its own: there is no built-in retry or backoff. `Participant.run()` iterates the adapter's stream once and does not catch or retry a transport error — the reachable "reconnect" is a caller invoking `start()` again, directly or after `stop()`.
